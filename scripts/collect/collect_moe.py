"""교육부 재정·예산 정보 게시판 어댑터 실행 검증 스크립트.

실제 사이트에서 문서 몇 건을 수집해 스키마 검증 + 첨부파일 다운로드 + 저장까지
end-to-end로 확인한다. collect_molit.py와 동일한 구조 — 이 소스도 헤드리스 브라우저
...
from rd2.adapters.moe import MoeAdapter  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="진행 상황 저장 파일 경로 (기본: rd2.db.moe_checkpoint.json)",
    )
    parser.add_argument(
    ...
    checkpoint_path = (
        Path(args.checkpoint) if args.checkpoint else repo_root / "rd2.db.moe_checkpoint.json"
    )
