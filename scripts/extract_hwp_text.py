...
data/extracted/ 아래 파일 1개당 JSON 1개로 저장한다(LibreOffice 변환 없음).

`scripts/extract_pdf_text.py`와 완전히 동일한 출력 스키마(source_pdf_path/
pages/spans)를 쓴다 — annotate.py/find_candidates.py는 소스가 PDF인지
HWP인지 몰라도 그대로 동작한다.
