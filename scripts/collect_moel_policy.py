"""고용노동부(moel.go.kr) 정책자료실 어댑터 실행 검증 스크립트.

실제 사이트에서 문서 몇 건을 수집해 스키마 검증 + 첨부파일 다운로드 + 저장까지
end-to-end로 확인한다. collect_moel.py와 동일한 구조.
"""
...
from rd2.adapters.moel_policy import MoelPolicyAdapter  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="진행 상황 저장 파일 경로 (기본: rd2.db.moel_policy_checkpoint.json)",
    )
    parser.add_argument(
    ...
    checkpoint_path = (
        Path(args.checkpoint) if args.checkpoint else repo_root / "rd2.db.moel_policy_checkpoint.json"
    )
    ...
    with DocumentStore() as store:
        try:
            for raw_item in adapter.fetch_list(skip=base_skip, max_items=args.count):
                ...
                print(f"    dept={doc.department!r} doc_type={doc.doc_type!r}")
