"""국토교통부 정책정보 게시판 어댑터 실행 검증 스크립트.

실제 사이트에서 문서 몇 건을 수집해 스키마 검증 + 첨부파일 다운로드 + 저장까지
end-to-end로 확인한다. collect_mohw.py와 동일한 구조 — 이 소스도 헤드리스
브라우저 없이 순수 httpx로 동작하고 클릭 검증이 필요 없어 체크포인트는 있지만
skip 기반 재개만으로 충분하다. 다만 첨부파일이 수십MB 단위로 클 수 있어(실측
최대 62.8MB) mohw보다 건당 소요 시간이 길고 중간에 끊길 확률이 높다는 점을
고려해 체크포인트는 그대로 유지한다.
"""
...
from rd2.adapters.molit import MolitAdapter  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="진행 상황 저장 파일 경로 (기본: rd2.db.molit_checkpoint.json)",
    )
    parser.add_argument(
    ...
    # 파일명은 SQLite 시절 그대로 유지 — DB가 더 이상 파일이 아니지만, 기존
    # 체크포인트 파일과의 연속성을 위해 이름을 그대로 둔다(마이그레이션 당일 진행 중이던
    # 크롤링을 그대로 이어받기 위함, TODOS 참고).
    checkpoint_path = (
        Path(args.checkpoint) if args.checkpoint else repo_root / "rd2.db.molit_checkpoint.json"
    )
    ...
    with DocumentStore() as store:
        try:
            for raw_item in adapter.fetch_list(skip=base_skip, max_items=args.count):
                ...
                print(f"    agency={doc.ordering_agency!r} dept={doc.department!r} category={doc.subject_category!r}")
