...
추출하는 scripts/extract/extract_documents.py부터 실행한다. 별도 annotated 사본이나
...
사용 예:
    python scripts/augment/run_pipeline.py --clause 5 --limit 1 --dry-run   # 프롬프트만 확인
    python scripts/augment/run_pipeline.py --clause 5 --limit 3             # 실제 3건 치환(과금)
    python scripts/augment/run_pipeline.py --clause all --limit 1 --dry-run
    python scripts/augment/run_pipeline.py --clause 8 --from-scratch --source all --limit 5
"""
...
def main() -> None:
    ...
    if args.from_scratch:
        extraction_command = [_PY, "scripts/extract/extract_documents.py", "--source", args.source]
        if args.force:
            extraction_command.append("--force")
        _run(extraction_command)

    candidate_command = [_PY, "scripts/augment/find_candidates.py", "--clause", args.clause]
    ...
    for clause_no in clauses:
        cmd = [_PY, "scripts/augment/run_llm_augment.py", "--clause", clause_no, "--limit", str(args.limit)]
        if args.dry_run:
            cmd.append("--dry-run")
        if args.force:
            cmd.append("--force")
