"""기후에너지환경부(me.go.kr) 행정규칙(고시·훈령·예규) 어댑터 실행 검증 스크립트.

실제 사이트에서 문서 몇 건을 수집해 스키마 검증 + 저장까지 end-to-end로 확인한다.
collect_moe.py와 동일한 구조 — 이 소스도 헤드리스 브라우저 없이 순수 httpx로
동작하고 클릭 검증이 필요 없어 체크포인트는 있지만 skip 기반 재개만으로 충분하다.
이 어댑터는 본문파일을 저장하지 않는 메타데이터 전용 어댑터라 첨부파일 관련
출력이 없다(open_go_kr과 동일한 성격).
"""
...
from rd2.adapters.me import MeAdapter  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="진행 상황 저장 파일 경로 (기본: rd2.db.me_checkpoint.json)",
    )
    parser.add_argument(
    ...
    checkpoint_path = (
        Path(args.checkpoint) if args.checkpoint else repo_root / "rd2.db.me_checkpoint.json"
    )
    ...
    with DocumentStore() as store:
        try:
            for raw_item in adapter.fetch_list(skip=base_skip, max_items=args.count):
                ...
                print(f"    agency={doc.ordering_agency!r} doc_type={doc.doc_type!r}")
