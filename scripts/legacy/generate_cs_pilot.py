...
from rd2.generators.generate import (
    SpanSeedInput,
    default_client,
    generate_clause_document,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt < max_attempts:
                time.sleep(2**attempt if is_rate_limited(exc) else 1.0)
...
def main() -> None:
    ...
    with output_path.open(mode, encoding="utf-8", newline="") as f:
        ...
        if args.admin_status_sample_only:
            ...
            # 읽기 전용이다 — 이 파일 어디에도 commit()/INSERT/UPDATE가 없다.
