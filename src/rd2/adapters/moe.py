...
- 담당부서는 거의 항상 채워지지만(113건 전수 크롤링 기준 112/113), boardSeq=52286
  (2014년도 예산 및 기금운용계획 개요 게시물)은 원본 HTML 자체가 담당부서 `<td>`를
  빈 채로 등록해뒀다(어댑터 파싱 버그 아님, 원문 직접 확인) — mohw의
  start_date/end_date와 같은 성격이라 conformance.py의 SOURCE_MOE 계약에서
  department를 always_filled에 넣지 않는다.
