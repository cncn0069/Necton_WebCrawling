...
def test_open_item_downloads_click_verified_files_and_picks_title_match(monkeypatch, tmp_path):
    ...
    assert (tmp_path / SOURCE_PRISM / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-1_정책연구과제_심의신청서(샘플).pdf").exists()
    assert (tmp_path / SOURCE_PRISM / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-1_공개 연구 샘플.pdf").exists()
    assert doc.body_file_path == str(
        Path(SOURCE_PRISM) / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-1_공개 연구 샘플.pdf"
    )
    assert doc.other_file_paths == [
        str(Path(SOURCE_PRISM) / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-1_정책연구과제_심의신청서(샘플).pdf")
    ]
...
def test_download_files_for_reruns_click_verification_later(monkeypatch, tmp_path):
    ...
    assert body_file_path == str(Path(SOURCE_PRISM) / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-1_공개 연구 샘플.pdf")
...
def test_partial_item_downloads_only_click_verified_subset(monkeypatch, tmp_path):
    ...
    doc = adapter.to_schema(detail)
    assert doc.body_file_path == str(
        Path(SOURCE_PRISM) / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-5_정책연구_활용결과_보고서.pdf"
    )
    assert doc.other_file_paths == []
    assert not (tmp_path / SOURCE_PRISM / DOC_TYPE_RESEARCH_REPORT / "1-500" / "3330000-5_[요약본]부분공개 샘플.pdf").exists()
