...
각 표를 보존하며, 전체 원문을 자연 배치해 검증한 뒤 최종 PDF는 최대 12쪽만
남긴다.
보도자료 경로는 기관명이 없을 때 가상 기관을 채우지 않고 헤더의 기관 영역을
비운다. `보도자료` 라벨과 페이지 번호만 서식 요소로 추가하고, 보도시점·배포일,
...
보존한다. 전체 원문 검증 후 최종 PDF는 최대 12쪽만 남긴다.
