...
| 서울 정보소통광장 | `seoul_opengov` | `seoul_opengov.py` | 결재문서 원문정보(`/sanction` — sanction은 행정처분이 아니라 결재라는 뜻). 공개/부분공개/비공개가 목록·상세에 명시돼 O/S/C가 한 소스에서 나온다(2026-07-16 추가) |
...
| seoul_opengov | 공문(결재 통지) | `official_document`(open_go_kr와 공유) | 고정값 — 제목(지급/출장/승인...)과 무관하게 실제 수집물은 전부 "안건을 알리는 결재 통지 커버 문서"다. 안건의 실체 첨부(내역서/조사서 등)는 비공개라 수집되지 않으므로 제목 키워드 분류는 내용과 어긋난다(2026-07-16 뷰어 본문 실사 후 사용자 결정) |
